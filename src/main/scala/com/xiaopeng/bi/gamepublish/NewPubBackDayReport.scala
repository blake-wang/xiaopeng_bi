...
/**
 * Created by JSJSB-0071 on 2017/5/17.
 * bdk.new_login_account_num:new_regi_device_login_num==>dev_add_lg_times:new_regi_device_login_num
 * pay_people_num:order_num
...
object NewPubBackDayReport {
  def main(args: Array[String]) {
    ...
    while ( resultSet.next() ) {
      ...
      if(!pkgCode.contains("\\")){
        /*val payPeopleNum2Sql = "select count(imei) from bi_gamepublic_pay_device_detail where game_id = " +gameId+" and pkg_code = '"+pkgCode+"' and medium_channel='"+medium_channel+"' and ad_site_channel='"+ad_site_channel+"' and publish_date >'"+publishDate+"' and  publish_date <='"+day2+"'"
        val payPeopleNum3Sql = "select count(imei) from bi_gamepublic_pay_device_detail where game_id = " +gameId+" and pkg_code = '"+pkgCode+"' and medium_channel='"+medium_channel+"' and ad_site_channel='"+ad_site_channel+"' and publish_date >'"+publishDate+"' and  publish_date <='"+day3+"'"
        val payPeopleNum7Sql = "select count(imei) from bi_gamepublic_pay_device_detail where game_id = " +gameId+" and pkg_code = '"+pkgCode+"' and medium_channel='"+medium_channel+"' and ad_site_channel='"+ad_site_channel+"' and publish_date >'"+publishDate+"' and  publish_date <='"+day7+"'"
        println(payPeopleNum2Sql)
        println(payPeopleNum3Sql)
        println(payPeopleNum7Sql)
        val resultDay2Set = statement2.executeQuery(payPeopleNum2Sql)
        val resultDay3Set = statement3.executeQuery(payPeopleNum3Sql)
        val resultDay7Set = statement7.executeQuery(payPeopleNum7Sql)

        val order_num_1day = getPayPeopleNumResult(resultDay2Set)
        val order_num_3day= getPayPeopleNumResult(resultDay3Set)
        val order_num_7day = getPayPeopleNumResult(resultDay7Set)*/
