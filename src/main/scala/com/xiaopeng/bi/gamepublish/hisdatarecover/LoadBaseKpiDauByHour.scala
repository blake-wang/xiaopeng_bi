...
object LoadBaseKpiDauByHour {
  def main(args: Array[String]): Unit = {
      //日志输出模式
       Logger.getLogger("org.apache.spark").setLevel(Level.WARN)
       Logger.getLogger("org.eclipse.jetty.server").setLevel(Level.OFF)

      val hivesql="select child_game_id game_id,medium_channel parent_channel,ad_site_channel child_channel,pkg_code ad_label,publish_date publish_time,parent_game_id,group_id,os,dau_account_num_hour dau_account_num from default.basekpi_hour"
      val mysqlsql = "insert into bi_gamepublic_basekpi(game_id,parent_channel,child_channel,ad_label,publish_time,parent_game_id,group_id,os,dau_account_num)" +
        " values(?,?,?,?,?,?,?,?,?)" +
        " on duplicate key update dau_account_num=?,os=?,group_id=?"
      //全部转为小写，后面好判断
