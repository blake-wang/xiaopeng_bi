...
object LoadBaseOperaKpi3Q {
  def main(args: Array[String]): Unit = {
      //日志输出模式
       Logger.getLogger("org.apache.spark").setLevel(Level.WARN)
       Logger.getLogger("org.eclipse.jetty.server").setLevel(Level.OFF)

      val hivesql="select child_game_id,publish_date,parent_game_id,group_id,os,dau_device_num,new_login_account_num,new_login_device_num,new_pay_people_num,new_pay_account_num,new_pay_money from default.day_kpi3"
      val mysqlsql = "insert into bi_gamepublic_base_opera_kpi(child_game_id,publish_date,parent_game_id,group_id,os,dau_device_num,new_login_account_num,new_login_device_num,new_pay_people_num,new_pay_account_num,new_pay_money)" +
        " values(?,?,?,?,?,?,?,?,?,?,?)" +
        " on duplicate key update dau_device_num=?,new_login_account_num=?,new_login_device_num=?,new_pay_people_num=?,new_pay_account_num=?,new_pay_money=?,os=?,group_id=?,parent_game_id=?"
      //全部转为小写，后面好判断
