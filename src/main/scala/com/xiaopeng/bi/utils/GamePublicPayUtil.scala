...
object GamePublicPayUtil {
  ...
  def loadPayInfo(rdd: RDD[String], sqlContext: SQLContext) = {
    StreamingUtils.convertOrderToDfTmpTable(rdd, sqlContext)
    // 运营报表 充值设备数
    val payPeopleDf = sqlContext.sql("select distinct split(publish_time,' ')[0],game_id,imei,game_account from ods_order")
    foreachPartitionPayDev(payPeopleDf)
    // 投放报表 充值账户数,累计充值账户数
    val payAccountDf = sqlContext.sql("select distinct publish_time,game_id,game_account from ods_order")
    foreachPartitionPayAccount(payAccountDf)
    // 投放报表 充值金额，累计充值金额，ltv
    val payMoneyDf = sqlContext.sql("select publish_time,game_id,game_account,ori_price,total_amt from ods_order")
    foreachPartitionPayMoney(payMoneyDf)

  }


  private def foreachPartitionPayDev(payPeopleDf: DataFrame) = {
  ...
  private def foreachPartitionPayAccount(payAccountDf: DataFrame) = {
  ...
  private def foreachPartitionPayMoney(payAccountDf: DataFrame) = {
    payAccountDf.foreachPartition(rows => {
      if (!rows.isEmpty) {
        //创建jedis客户端
        val pool: JedisPool = JedisUtil.getJedisPool;
  ...
  def insertPayDevNum(rows: Iterator[Row], jedis: Jedis, conn: Connection, statement: Statement) = {
      ...
      if (rs.next()) {
        if (rs.getInt(1) == 0) {
          ...
          if (channelArray(0).length <= 10 && channelArray(1).length <= 10 && channelArray(2).length <= 15) {
            //publish_time,game_id,imei,game_account
    ...
    try {
      ...
      case ex: Exception => {
        logger.error("=========================插入foreachPartitionPayPeople表异常：" + ex)
      }
    }
  }

  private def insertPayAccountNum(rows: Iterator[Row], jedis: Jedis, conn: Connection, statement: Statement) = {
    if (!rows.isEmpty) {
      //投放天表
      ...
      for (row <- rows) {
        ...
        if (rs_all.next()) {
          if (rs_all.getInt(1) == 0) {
            if (channelArray(0).length <= 10 && channelArray(1).length <= 10 && channelArray(2).length <= 15) {
              //历史不存在
              //运营天表  累计充值帐号数  将计算结果累加到帐号注册那天上      这个字段是用于刷榜
      ...
      try {
        ...
        case ex: Exception => {
          logger.error("=========================插入insertPayAccountNum表异常：" + ex)
        }
      }
    }
  }

  private def insertPayMoney(rows: Iterator[Row], jedis: Jedis, conn: Connection, statement: Statement) = {
    //投放天表  充值金额 pay_money
    ...
    for (insertedRow <- rows) {
      ...
      if (channelArray(0).length <= 10 && channelArray(1).length <= 10 && channelArray(2).length <= 15) {
        ...
        //投放天表   充值金额  按天存入
        dayParams.+=(Array[Any](order_date, insertedRow.get(1), channelArray(0), channelArray(1), channelArray(2)
        ...
        //运营天表  当天账号注册日N天后累计充值金额，计算结果累加到帐号注册那天
    ...
    try {
      ...
      case ex: Exception => {
        logger.error("=========================插入insertPayMoney表异常：" + ex)
      }
    }
  }
