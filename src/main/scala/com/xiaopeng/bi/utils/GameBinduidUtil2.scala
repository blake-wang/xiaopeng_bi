...
object GameBinduidUtil2 {
  ...
  def loadBinduidInfo(rdd: RDD[String], hiveContext: HiveContext) = {
    val binduidRdd = rdd.filter(line => {
      ...
      fields(0).contains("bi_binduid") && fields.length >= 6 && (!fields(2).equals(""))
    }).map(line => {
      println("bi_binduid" + line)
      val fields = line.split("\\|", -1)
  ...
  def foreachBinduidDataFrame(binduidDataFrame: DataFrame) = {
    binduidDataFrame.foreachPartition(iter => {
      if (!iter.isEmpty) {
        ...
        val member_game_account_num = "insert into bi_member_info (member_id,accounts) values (?,?) on duplicate key update accounts = accounts + 1"
        ...
        iter.foreach(row => {
          ...
          member_game_account_num_params.+=(Array(member_id, accounts))
