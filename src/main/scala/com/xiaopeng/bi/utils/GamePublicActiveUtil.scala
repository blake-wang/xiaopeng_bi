...
object GamePublicActiveUtil {
  ...
  def loadActiveInfo(rdd: RDD[String], hiveContext: HiveContext) = {
    ...
    val activeRdd = rdd.filter(t => {
      ...
      // game_id      parent_channel   child_channel   ad_label  active_time  imei
  ...
  def foreachRealPartition(df_bi_active: DataFrame) = {
    df_bi_active.foreachPartition(iter => {
      if (!iter.isEmpty) {
        ...
        val ps_hour_active_num: PreparedStatement = conn.prepareStatement(sql_hour_active_num);
        ...
        val ps_opera_hour_kpi: PreparedStatement = conn.prepareStatement(sql_opera_hour_kpi);
        ...
        val ps_opera_kpi: PreparedStatement = conn.prepareStatement(sql_opera_kpi);
        ...
        iter.foreach(row => {
          ...
          if (parent_channel.length <= 10 && child_channel.length <= 10 && ad_label.length <= 12) {
            ...
            if (!results_all.next()) {
              // 明细表  :  设备在分包的第一次激活时插入一条明细
              params_deatil.+=(Array[Any](child_game_id, parent_channel, child_channel, ad_label, publish_time, imei))
              // 投放小时表  ： 设备在该小时是历史第一次激活算一个
              ...
              if (DateUtils.beforeHour(publish_time, active_hour_old)) {
                // 新数据的激活时间靠前
                // 更新明细表
                params_deatil_update.+=(Array[Any](publish_time, child_game_id, parent_channel, child_channel, ad_label, imei))
                // 投放小时表
                params_hour_active.+=(Array[Any](parent_game_id, child_game_id, parent_channel, child_channel, ad_label, publish_time, 1, os, group_id, medium_account, promotion_channel, promotion_mode, head_people))
            ...
            if (!results_game_all.next()) {
              ...
              if (DateUtils.beforeHour(publish_time, active_hour_old)) {
                ...
                //运营天表（日报）
                params_opera_kpi.+=(Array[Any](parent_game_id, child_game_id, publish_time.substring(0, 10), 1, os, group_id))
                params_opera_kpi.+=(Array[Any](parent_game_id, child_game_id, active_hour_old.substring(0, 10), -1, os, group_id))
              }
            }
          }
          // 插入数据库
