...
object ThirdDataDao1 {
  ...
  /**
    ...
    * @return
    */
  def checkAndroidIsMatched(game_id: Int, androidID: String, conn: Connection, stmt: Statement): Boolean ={
    var statusCode=false;
    val selectImei = "select imei from bi_ad_active_o_detail where game_id='" + game_id + "' and androidID='" + androidID + "' limit 1"
    val imeiResultSet = stmt.executeQuery(selectImei)
    if (imeiResultSet.next()) {
      //androidID检测到激活 -->不再执行匹配动作
      logger.info("androidID已经匹配成功，不再进行匹配，设备为："+androidID);
      statusCode = true
    }
    return  statusCode
  ...
  def insertOrderStat(orderDate: String, gameId: Int, group_id: String, pkgCode: String, head_people: String, medium_account: String, medium: Int,
                      idea_id: String, first_level: String, second_level: String, payPrice: Float, payAccs: Int, newPayPrice: Float, newPayAccs: Int, conn: Connection) = {
  ...
  def insertRegiStat(regiDate: String, gameId: Int, group_id: String, pkgCode: String, head_people: String, medium_account: String, medium: Int,
                     idea_id: String, first_level: String, second_level: String, regiNum: Int, regiDev: Int, newRegiNum: Int, conn: Connection) = {
    ...
    val sql2Mysql = "insert into bi_ad_channel_stats" +
      ...
      " values(?,?,?,?,?,?,?,?,?,?,?,?) " +
      " on duplicate key update regi_num=regi_num+?,regi_dev_num=regi_dev_num+?,group_id=?,head_people=?,medium_account=?,new_regi_dev_num=new_regi_dev_num+?"
    val ps: PreparedStatement = conn.prepareStatement(sql2Mysql)
    /*insert*/
    ...
    /*update*/
    ps.setInt(13, regiNum)
    ps.setInt(14, regiDev)
    ps.setString(15, group_id)
    ps.setString(16, head_people)
    ps.setString(17, medium_account)
    ps.setInt(18, newRegiNum)
    ps.executeUpdate()
    ps.close()
  ...
  def insertActiveStat(activeDate: String, gameId: Int, group_id: String, pkgCode: String, head_people: String, medium_account: String, medium: Int,
                       idea_id: String, first_level: String, second_level: String, activeNum: Int, channel_main_id: Int, channel_name: String, conn: Connection) = {
    val sql2Mysql = "insert into bi_ad_channel_stats" +
      "(publish_date,game_id,group_id,pkg_id,head_people,medium_account,medium,idea_id,first_level,second_level,active_num,channel_main_id,channel_name)" +
      " values(?,?,?,?,?,?,?,?,?,?,?,?,?) " +
  ...
  def insertClickStat(clickDate: String, gameId: Int, group_id: String, pkgCode: String, head_people: String, medium_account: String, medium: Int, channel_main_id: Int, channel_name: String,
                      idea_id: String, first_level: String, second_level: String, clicks: Int, clickDevs: Int, conn: Connection) = {
    val sql2Mysql = "insert into bi_ad_channel_stats" +
      "(publish_date,game_id,group_id,pkg_id,head_people,medium_account,medium,channel_main_id,channel_name,idea_id,first_level,second_level,click_num,click_dev_num)" +
      " values(?,?,?,?,?,?,?,?,?,?,?,?,?,?) " +
      " on duplicate key update click_num=click_num+?,click_dev_num=click_dev_num+?,head_people=?,medium_account=?,channel_main_id=?,channel_name=?"
    val ps: PreparedStatement = conn.prepareStatement(sql2Mysql)
    /*insert*/
    ...
    /*update*/
    //因为出现过bi_ad_channel_stats数据表中未插入head_people和medium_account的情况，因此每次插入bi_ad_channel_stats表数据时都再更新一次head_people和medium_account
    ps.setInt(15, clicks)
    ps.setInt(16, clickDevs)
    ps.setString(17, head_people)
    ps.setString(18, medium_account)
    ps.setInt(19, channel_main_id)
    ps.setString(20, channel_name)
    ps.executeUpdate()
    ps.close()
  ...
  /**
    ...
    * @return
    */
  def iosMatchClickByIPAndUA(gameId: Int, activeDate6dayBefore: String, activeDate: String, ip: String, devType: String, sysVersion: String, conn: Connection): Tuple10[Int, String, String, String, Int, String, String, String, Int, String] = {
    var tp10 = Tuple10(0, "", "", "", 0, "", "", "", 0, "")
    var pkg_id = ""
    var adv_name = 0
    var ideaId = ""
    ...
    jg = ps.executeUpdate()
    if (jg > 0) {
      logger.info("ios激活通过IP+UA匹配成功 : " + ip + "|" + devType + "|" + sysVersion + ", gameid:" + gameId)
