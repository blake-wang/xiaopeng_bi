        ...
        part.foreach(line => {
          ...
          if (!clickTime.equals("0")) {
            ...
            if (pkg_id.matches(regx)) {
              ...
              ThirdDataDao.insertClickStat(clickDate, game_id, group_id, pkg_id, head_people, medium_account, advName, channel_main_id, channel_name, idea_id, first_level, second_level, click_num, click_dev_num,remark, conn)
            }
          }
        })
          ...
          if (osInt == 2) {
            ...
            if (!imei.equals("") && !imei.matches("[0]{7,}")) {
              ...
              if (statusCode) {
                ...
                matched = ThirdDataDao.iosMatchClickByImei(imei, imeiWithLine_md5_upper, imei_md5_upper, game_id, activeDate6dayBefore, activeDate, conn)
                  ...
                  if (!ip.equals("")) {
                    ...
                    if (!devType.equals("")) {
                      ...
                      if (!sysVersion.equals("")) {
                        ...
                        matched = ThirdDataDao.iosMatchClickByIPAndDevAndSys(game_id, activeDate6dayBefore, activeDate, ip, devType, sysVersion, conn)
                        if (matched._1 == 0) {
                          //game_id + ip + devType + sysVersion 没有匹配到  --> 执行 game_id + ip + devType 匹配
                          matched = ThirdDataDao.iosMatchClickByIPAndDev(game_id, activeDate6dayBefore, activeDate, ip, devType, conn)
                          if (matched._1 == 0) {
                            //game_id + ip + devType 没有匹配到  --> 执行 game_id + ip 匹配
                            matched = ThirdDataDao.iosMatchClickByIP(game_id, activeDate6dayBefore, activeDate, ip, conn)
                          }
                        }
                      } else {
            ...
            if (!imei.equals("") && !imei.matches("[0]{7,}")) {
              ...
              if (statusCode) {
                //imei有效
                val imei_md5_upper = MD5Util.md5(imei).toUpperCase
                matched = ThirdDataDao.androidMatchClickByImei(imei_md5_upper, game_id, activeDate6dayBefore, activeDate, conn)
                if (matched._1 == 0) {
                  ...
                  if (!androidID.equals("")) {
                    ...
                    if (statusCode) {
                      ...
                      matched = ThirdDataDao.androidMatchClickByAndroidID(androidID, game_id, activeDate6dayBefore, activeDate, conn)
                    }
                  }
                }
              }
      ...
      newRegiRdd.foreachPartition(part => {
        ...
        part.foreach(line => {
          ...
          if (adv_name != 0) {
            ...
            //写入广告监测平台注册明细
            ThirdDataDao.insertRegiDetail(regiTime, imei, pkgCode, adv_name, gameId, osInt, gameAccount, channel_main_id, channel_name, androidID, conn)
            //注册统计
            //            ThirdDataDao.insertRegiStat(regiDate, gameId, group_id, pkgCode, head_people, medium_account, adv_name, idea_id, first_level, second_level, regiNum, regiDevNum, newRegiDevNum, conn)
        ...
        part.foreach(line => {
          ...
          if (!jedis6.exists("adv_orderexists|" + gameAccount + "|" + orderID + "|" + orderTime)) {
            ...
            if (advName != 0) {
              ...
              //消费明细
              ThirdDataDao.insertOrderDetail(orderID, orderTime, imei, pkgCode, medium, gameId, os, gameAccount, payPrice, channel_main_id, channel_name, conn)
              //消费统计
