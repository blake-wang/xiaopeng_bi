    ...
    df.foreachPartition(iter => {
      if (!iter.isEmpty) {
        ...
        iter.foreach(row => {
          ...
          //  激活注册设备数  <新广告监测用到>
          var base_opera_kpi_active_regi_dev_num = 0
          ...
          if (parent_channel.length <= 10 && child_channel.length <= 10 && ad_label.length <= 12) {
            ...
            if (!results_oper_all.next()) {
              ...
              //激活注册设备数  运营天表 (日报)
              val sql_active = "select active_hour from bi_gamepublic_active_detail where game_id='" + child_game_id + "' and imei='" + imei + "' and date(active_hour)='" + publish_time.substring(0, 10) + "'"
              val res = stmt.executeQuery(sql_active)
              if (res.next()) {
                base_opera_kpi_active_regi_dev_num = 1
              }
            } else {
          ...
          if (!(base_opera_kpi_new_regi_device_num == 0 && base_opera_kpi_new_regi_account_num == 0 && base_opera_kpi_regi_device_num == 0)) {
            params_base_opera_kpi.+=(Array[Any](parent_game_id, child_game_id, publish_time.substring(0, 10), base_opera_kpi_new_regi_device_num, base_opera_kpi_new_regi_account_num, base_opera_kpi_regi_device_num,base_opera_kpi_active_regi_dev_num, os, group_id))
